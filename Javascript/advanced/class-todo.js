...
class View {
  ...
  // createElement   //tao ra cac the tuw crelemt 
  createElement(tag, className) {
    const elm = document.createElement(tag);
    if (className) elm.classList.add(className);
    return elm;
  }
  // getElement laay ra the
  getElement(selector) {
    const elm = document.querySelector(selector);
    return elm;
  }
 //todoValie lay ra gia tri cua input
  get _todoValue() {
    return this.input.value;
  }
// reset value cua input lai
  _resetValue() {
    this.input.value = "";
  }
  //hien thi gia tri truyen vao
  displayTodos(todos) {
  ...
  //thuc hien chuc nag  khi form submit
  viewAddTodo(handler) {
