/**
 * status: pending //khoi tao 
 * status: resolve // kthah cong
 * status: reject //   ko thanh cong 
 */  //promise chi dung  resolve va reject chay khong thi trang thai pending
...
//then là thành công , catch thất bại , finally dù thất bại hay thành công đều đc ;
...
//Promise.all chay lan luot tra ve tat ca thanh cong  ;
const timerTotal = Promise.all([timer1, timer2, timer3]).then((data) => {
  // console.log(data);
});
// Promise.race chay luon lap tuc 
const timerTotal2 = Promise.race([timer1, timer2, timer3]).then((data) => {
  // console.log(data);
});
// Promise.allSettled tra ve  ca 2 that bai hay thnah cong 
